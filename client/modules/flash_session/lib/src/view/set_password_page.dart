...
/// 设置密码页面 — 仅用于首次设置，单一输入框，调用 setPassword
...
class _SetPasswordPageState extends State<SetPasswordPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await context.read<SessionCubit>().setPassword(password);
      if (!mounted) return;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Text('为账号设置一个密码', style: TextStyle(fontSize: 15, color: Color(0xFF888888))),
          const SizedBox(height: 32),
          LabeledInput(controller: _passwordCtrl, label: '新密码', hintText: '请输入密码（至少 6 位）',
            ...
            suffix: GestureDetector(onTap: () => setState(() => _obscurePassword = !_obscurePassword),
              child: Padding(padding: const EdgeInsets.only(right: 4),
                child: Icon(_obscurePassword ? Icons.visibility_off_outlined : Icons.visibility_outlined, size: 22, color: const Color(0xFFAAAAAA)))),
          ),
