...
8. ✅ 任务 8 — 编译验证 + 测试路径（依赖全部）
   - ✅ 8.1 编译通过
   - ✅ 8.2 端到端测试（10 tests: AuthState/AuthCubit/StartupRepository/User 模型）
...
测试文件：`client/flash_im/test/features/starter/starter_e2e_test.dart`

覆盖 10 个测试点：
1. AuthState 三种状态（unknown/authenticated/unauthenticated）正确构造
2. AuthCubit applyStartupSnapshot → onPasswordSet → unauthenticated 状态转换
3. AuthCubit logout 状态转换（authenticated → unauthenticated）
4. StartupRepository 正确读取 SharedPreferences（有 Token → authenticated）
5. StartupRepository 无 Token → unauthenticated
6. StartupResult.authenticated getter
7. User fromJson / toJson 完整性
8. AuthState Equatable props 正确性
9. AuthCubit.login 方法更新状态
10. 完整启动→登录→退出流程（纯逻辑端到端）
