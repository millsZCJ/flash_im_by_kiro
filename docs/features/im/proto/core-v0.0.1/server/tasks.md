...
说明：
- `flash_core`：复用 verify_token 与 Config
- `axum`：WebSocket 支持
- `tokio`：异步运行时、timeout
...
async fn wait_for_auth(
    ...
    // 5. 调用 flash_core::jwt::verify_token(&auth_req.token, &jwt_secret)
...
删除 `server/src/ws.rs` 以及 `server/src/chat_room.rs`（旧广播聊天室）。

删除 `server/src/main.rs` 中的 `mod ws;`、`mod chat_room;`、`chat_room::chat_room_handler` 以及 `/ws`、`/chat_room` 路由。

### 5.2 清理 AppState `✅`

文件：`server/src/main.rs`（修改）

移除 `room_tx: broadcast::Sender<RoomMessage>` 字段和 `new_broadcast()` 创建。
