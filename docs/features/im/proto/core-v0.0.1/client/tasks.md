...
文件：`client/flash_im/lib/main.dart`（修改）
...
通过 `BlocListener<AuthCubit, AuthState>` 监听认证状态：
...
注意：退出登录时 `AuthStatus.unauthenticated` 会触发 `wsClient.disconnect()`，不再重连。

---

## 任务 7：home_page.dart — 集成指示器 `✅`

文件：`client/flash_im/lib/src/home/view/home_page.dart`（修改）
